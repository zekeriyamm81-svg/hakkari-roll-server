...
async function getModelsForBrand(name){
 const m=findVehicleMake(name);const key=m?.id?`id:${m.id}`:`name:${String(name||'').toLocaleLowerCase('tr-TR')}`;
...
function setProfileTab(tab,btn){
 ...
 // Mobilde sekme değişince paneli sabit üst barın ALTINA hizala; nested scroll oluşmasın.
 if(window.matchMedia('(max-width:820px)').matches){requestAnimationFrame(()=>{const tabs=document.querySelector('.v53-profile-tabs'),panel=$(panels[tab]);if(!tabs||!panel)return;const topbar=$('topbar')||document.querySelector('.v5-topbar');const offset=(topbar?.getBoundingClientRect().height||62)+8;const y=tabs.getBoundingClientRect().top+window.scrollY-offset;if(Math.abs(window.scrollY-y)>6)window.scrollTo({top:Math.max(0,y),behavior:'smooth'});});}
}
...
async function openConversation(id,title="Sohbet"){stopMessageLive();activeConversation=id;lastMessageId=0;messagePollTick=0;const c=conversationCache.find(x=>x.id===id),o=c?.other;const callBtn=$('chatCallBtn');if(callBtn){callBtn.classList.toggle('hidden',!o?.id);callBtn.dataset.userId=o?.id||'';}$("chatTitle").textContent=o?.display_name||title||"Sohbet";if($("chatPeerAvatar"))$("chatPeerAvatar").innerHTML=img(o?.profile_photo);if($("chatPresence")){$("chatPresence").classList.remove("typing");$("chatPresence").textContent=conversationPresence(c)}if($("composerTyping")){$("composerTyping").classList.add("hidden");$("composerTyping").textContent=""}$("messagesPage")?.classList.add("chat-open");renderConversations();await loadMessages();loadConversations(true);messageTimer=setInterval(async()=>{await pollConversationLive();messagePollTick++;if(messagePollTick%4===0)loadConversations(true)},1100);setTimeout(()=>$('chatInput')?.focus(),80)}
...
const HR_SIMPLE_ICON_ALIASES={"Mercedes-Benz":"mercedes","Mercedes Benz":"mercedes","Alfa Romeo":"alfaromeo","Land Rover":"landrover","Range Rover":"landrover","Rolls-Royce":"rollsroyce","Aston Martin":"astonmartin","Volkswagen":"volkswagen","Škoda":"skoda","Skoda":"skoda","Citroën":"citroen","Citroen":"citroen","General Motors":"generalmotors","Polestar":"polestar","Lucid":"lucid","Rivian":"rivian","Genesis":"genesis","Bentley":"bentley","Bugatti":"bugatti","Ferrari":"ferrari","Lamborghini":"lamborghini","Maserati":"maserati","McLaren":"mclaren","Lotus":"lotus","Acura":"acura","Infiniti":"infiniti","Cadillac":"cadillac","GMC":"gmc","Ram":"ram","Chrysler":"chrysler","Dodge":"dodge","Buick":"buick","Saab":"saab","Smart":"smart","Geely":"geely","BYD":"byd","Chery":"chery","MG":"mg"};
function genericBrandLogoData(brand){const b=String(brand||'HR').trim();const letters=(b.split(/\s+/).map(x=>x[0]||'').join('').slice(0,3)||'HR').toUpperCase();const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" rx="24" fill="#151c26"/><circle cx="50" cy="50" r="39" fill="none" stroke="#d7dde6" stroke-width="5"/><text x="50" y="59" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#fff">${letters}</text></svg>`;return 'data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg)}
function brandLogoUrl(brand){
  ...
  if(key)return HR_REAL_BRAND_LOGOS[key];
  const alias=HR_SIMPLE_ICON_ALIASES[String(brand||'').trim()]||String(brand||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]/g,'');
  return alias?`https://cdn.jsdelivr.net/gh/simple-icons/simple-icons@16.28.0/icons/${alias}.svg`:genericBrandLogoData(brand);
}
function brandLogo(brand,cls="brand-logo"){
  const fallback=genericBrandLogoData(brand);return `<img class="${cls}" src="${brandLogoUrl(brand)}" alt="${esc(brand||"Araç")} marka rozeti" loading="lazy" onerror="this.onerror=null;this.src='${fallback}'">`;
}
...
async function voicePollState(){
 if(!voiceCall)return;try{const d=await api(`/api/calls/${voiceCall.id}`),c=d.call;if(!c)return;voiceCall={...voiceCall,...c,peer:c.peer||voiceCall.peer};
  if(c.status==='accepted'){voiceSetAcceptedUi();await voiceFetchSignals()}
...
async function voicePollIncoming(){
 if(!token||!me||voiceCall||voiceCleaning)return;try{const d=await api('/api/calls/poll'),c=d.call;if(!c)return;
  ...
  if(c.status==='ringing'){voiceOpenOverlay('incoming');voiceStartLoops();voiceNotifyIncoming(c)}
 }catch(e){}
}
...
openProfile=async function(id){const r=await _v610OpenProfile(id);const box=$('profileBox');if(box&&Number(id)!==Number(me?.id)&&!box.querySelector('.v610-profile-call')){const actions=box.querySelector('.v53-profile-actions,.person-actions');actions?.insertAdjacentHTML('beforeend',`<button class="v610-profile-call" onclick="startVoiceCallUser(${Number(id)})">📞 SESLİ ARA</button>`)}return r};
