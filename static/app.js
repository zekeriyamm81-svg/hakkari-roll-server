...
function buildNav(){
 const items=[
  ...
  ["messages","Mesajlar","messagesPage",loadConversations]
 ];
...
function renderConversations(){
 ...
 const list=conversationCache.filter(c=>{const hay=`${c.other.display_name||""} ${c.other.username||""} ${c.last_message?.body||""}`.toLocaleLowerCase("tr");return (!q||hay.includes(q))&&(!conversationUnreadOnly||c.unread>0)});
 host.innerHTML=list.map(c=>`<button class="conversation v5-conversation ${activeConversation===c.id?"active":""}" data-conversation-id="${c.id}" onclick="openConversation(${c.id},'${esc(c.other.display_name).replace(/'/g,"&#39;")}')"><span class="v5-conversation-avatar">${img(c.other.profile_photo)}</span><span class="v5-conversation-copy"><b>${esc(c.other.display_name)}</b><small>@${esc(c.other.username)}</small><em>${esc(c.last_message?.body||"Yeni sohbet")}</em></span><span class="v5-conversation-meta">${c.unread?`<strong>${c.unread}</strong>`:""}<small>${esc(c.last_message?.created_at||"")}</small></span></button>`).join("")||`<div class="v5-empty-conversations"><span>💬</span><p>${conversationUnreadOnly?"Okunmamış mesajın yok.":q?"Aramana uygun sohbet bulunamadı.":"Henüz bir sohbetin yok."}</p></div>`;
}
...
async function loadConversations(){try{const d=await api("/api/conversations");conversationCache=d.conversations;renderConversations()}catch(e){toast(e.message)}}
async function openConversation(id,title="Sohbet"){activeConversation=id;$("chatTitle").textContent=title;if($("chatPresence"))$("chatPresence").textContent="Özel sohbet";renderConversations();await loadMessages();loadConversations();clearInterval(messageTimer);messageTimer=setInterval(loadMessages,1800)}
async function loadMessages(){if(!activeConversation)return;try{const d=await api(`/api/conversations/${activeConversation}/messages`);$("chatMessages").innerHTML=d.messages.map(m=>`<div class="bubble ${m.sender_id===me.id?"me":""}">${esc(m.body)}<div class="bubble-time">${esc(m.created_at)}${m.sender_id===me.id?` ${m.read_at?"✓✓":"✓"}`:""}</div></div>`).join("");$("chatMessages").scrollTop=$("chatMessages").scrollHeight}catch(e){}}
async function sendMessage(){if(!activeConversation){toast("Önce bir sohbet seç.");return}const body=$("chatInput").value.trim();if(!body)return;try{await api(`/api/conversations/${activeConversation}/messages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({body})});$("chatInput").value="";await loadMessages();loadConversations()}catch(e){toast(e.message)}}
