...
let currentRollState=null,rollActionBusy=false,lastRollPosition=null,lastRollUpdateAt=0;
...
function toggleMobileMenu(force){
 const drawer=$("mobileSideMenu");if(!drawer)return;
 const open=typeof force==="boolean"?force:drawer.classList.contains("hidden");
 drawer.classList.toggle("hidden",!open);document.body.classList.toggle("mobile-menu-open",open);
 if(open)renderMobileMenu();
}
function closeMobileMenu(){toggleMobileMenu(false)}
function renderMobileMenu(){
 const host=$("mobileSideMenuLinks");if(!host||!me)return;
 const current=document.querySelector(".page:not(.hidden)")?.id||"homePage";
 const main=mainNavPage(current)||current;
 const go=(label,icon,js,page="",cls="")=>`<button class="${cls} ${page&&main===page?"active":""}" onclick="closeMobileMenu();${js}"><i>${icon}</i><span>${label}</span></button>`;
 host.innerHTML=`<div class="v514-menu-label">MENÜ</div>`+
  go("Ana Sayfa","⌂","showPage('homePage');loadFeed();loadV4Home()","homePage")+
  go("Roll Merkezi","⌖","showPage('rollPage');loadRollHub()","rollPage")+
  go("Canlı Harita","◉","showPage('mapPage');loadRollMap()","rollPage")+
  go("Piyasadakiler","◎","showPage('peoplePage');loadPeople()","rollPage")+
  go("Roll Teklifleri","⚡","showPage('offersPage');loadOffers()","rollPage")+
  go("Etkinlikler","◆","showPage('eventsPage');loadEvents()","rollPage")+
  go("Ekipler","♟","showPage('crewsPage');loadCrews()","rollPage")+
  go("Garaj","🚘","showPage('carsPage');setGarageMode('mine')","carsPage")+
  go("Mesajlar","💬","showPage('messagesPage');loadConversations()","messagesPage")+
  `<div class="v514-menu-label account">HESAP</div>`+
  go("Profilim","●",`openProfile(${me.id})`)+
  go("Uygulamayı Kur","↓","openPWAInstallGuide()")+
  (me.role==="admin"?go("Yönetim","◆","showPage('adminPage');loadAdmin()","","admin"):"")+
  go("Çıkış Yap","↪","logout()","","danger");
}
function buildNav(){
 const items=[
...
function showPage(id){
 ...
 document.body.classList.remove("nav-open");closeMobileMenu();
...
async function showApp(){
 ...
 setTimeout(()=>{startVoiceCallWatcher();refreshNotificationPermissionUI();showPWAInstall()},0);
}
...
function setRollBusy(busy,text=""){
 rollActionBusy=busy;const btn=$("rollSaveBtn"),state=$("rollActionState");
 if(btn){btn.disabled=busy;btn.textContent=busy?(text||"KAYDEDİLİYOR…"):(currentRollState?"GÜNCELLE":"AKTİF ET")}
 if(state){state.textContent=busy?(text||"İşlem yapılıyor…"):(currentRollState?"Ayar değişiklikleri konum beklemeden kaydedilir.":"İlk açılışta cihaz konumu alınır.")}
}
function openRollPanel(){
 const modal=$("rollModal");if(!modal)return;
 if(currentRollState){
  if($("rollStatus"))$("rollStatus").value=currentRollState.status||"Piyasadayım";
  if($("rollVisibility"))$("rollVisibility").value=currentRollState.visibility||"approx";
 }
 modal.classList.remove("hidden");setRollBusy(false);
}
function closeRollPanel(){$("rollModal")?.classList.add("hidden");setRollBusy(false)}
function getPositionOnce(options){return new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,options))}
async function getFastRollPosition(){
 if(lastRollPosition && Date.now()-lastRollPosition.ts<120000)return lastRollPosition;
 try{
  const p=await getPositionOnce({enableHighAccuracy:false,maximumAge:120000,timeout:3500});
  return lastRollPosition={lat:p.coords.latitude,lon:p.coords.longitude,accuracy:p.coords.accuracy,ts:Date.now()};
 }catch(first){
  const p=await getPositionOnce({enableHighAccuracy:true,maximumAge:30000,timeout:7000});
  return lastRollPosition={lat:p.coords.latitude,lon:p.coords.longitude,accuracy:p.coords.accuracy,ts:Date.now()};
 }
}
async function activateRoll(){
 if(rollActionBusy)return;
 const payload={minutes:+$("rollMinutes").value,status:$("rollStatus").value,visibility:$("rollVisibility").value};
 try{
  // Roll zaten aciksa GPS'i yeniden bekleme: durum/sure/hassasiyet aninda kaydedilir.
  if(currentRollState){
   setRollBusy(true,"GÜNCELLENİYOR…");
   const d=await api("/api/roll/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
   currentRollState={...currentRollState,...payload,expires_at:d.expires_at};
   toast(d.message);closeRollPanel();loadRollCount();startRollWatch();return;
  }
  if(!navigator.geolocation){toast("Konum desteklenmiyor.");return}
  setRollBusy(true,"KONUM ALINIYOR…");
  const pos=await getFastRollPosition();
  setRollBusy(true,"AKTİF EDİLİYOR…");
  const d=await api("/api/roll/activate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...payload,lat:pos.lat,lon:pos.lon,accuracy:pos.accuracy})});
  currentRollState={user_id:me.id,...payload,lat:pos.lat,lon:pos.lon,accuracy:pos.accuracy,expires_at:d.expires_at};
  toast(d.message);closeRollPanel();startRollWatch();loadRollCount();
 }catch(e){
  const msg=(e&&e.code===1)?"Konum izni verilmedi.":(e&&e.code===3)?"Konum alınması uzun sürdü. Konumu açıp tekrar dene.":(e.message||"Roll Aktif açılamadı.");toast(msg)
 }finally{setRollBusy(false)}
}
function rollDistanceMeters(a,b){if(!a||!b)return Infinity;const R=6371000,rad=x=>x*Math.PI/180,dLat=rad(b.lat-a.lat),dLon=rad(b.lon-a.lon),q=Math.sin(dLat/2)**2+Math.cos(rad(a.lat))*Math.cos(rad(b.lat))*Math.sin(dLon/2)**2;return 2*R*Math.asin(Math.sqrt(q))}
function startRollWatch(){
 stopRollWatch();if(!navigator.geolocation||!currentRollState)return;
 const exact=currentRollState.visibility==="exact";
 rollWatchId=navigator.geolocation.watchPosition(pos=>{
  const next={lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy:pos.coords.accuracy,ts:Date.now()};
  const elapsed=Date.now()-lastRollUpdateAt,moved=rollDistanceMeters(lastRollPosition,next);
  lastRollPosition=next;
  if(elapsed<25000&&moved<40)return;
  lastRollUpdateAt=Date.now();
  api("/api/roll/update",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lat:next.lat,lon:next.lon,accuracy:next.accuracy})}).catch(()=>{});
 },()=>{},{enableHighAccuracy:exact,maximumAge:15000,timeout:10000});
}
...
async function deactivateRoll(){if(rollActionBusy)return;try{setRollBusy(true,"KAPATILIYOR…");const d=await api("/api/roll/deactivate",{method:"POST"});stopRollWatch();currentRollState=null;toast(d.message);closeRollPanel();loadRollCount()}catch(e){toast(e.message)}finally{setRollBusy(false)}}
async function loadRollCount(){try{const d=await api("/api/roll/active"),mine=d.active.find(x=>x.user_id===me.id);currentRollState=mine||null;if($("liveCount"))$("liveCount").innerHTML=`<i></i> ${d.count} ROLL AKTİF`;if($("v2HeroLive"))$("v2HeroLive").textContent=`${d.count} CANLI`;if($("radarCount"))$("radarCount").textContent=`${d.count} kişi`;if($("v5RollCount"))$("v5RollCount").textContent=d.count;if($("v5RollStatus"))$("v5RollStatus").textContent=mine?`${mine.status} • Roll aktif`:"Şu an çevrimdışısın";if($("rollToggleBtn"))$("rollToggleBtn").innerHTML=mine?"🟢 ROLL AKTİF <b>→</b>":"ROLL AKTİF ET <b>→</b>";if($("v5RollToggleBtn"))$("v5RollToggleBtn").innerHTML=mine?"ROLL DURUMUNU YÖNET <b>→</b>":"ROLL AKTİF ET <b>→</b>"}catch(e){}}
...
// HAKKARI ROLL PWA — V5.14 ana sayfa kurulum rehberi
let deferredPWA=null;
const isStandalone=()=>window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===true;
const isIOS=()=>/iphone|ipad|ipod/i.test(navigator.userAgent);
const isAndroid=()=>/android/i.test(navigator.userAgent);
const isMobileInstallTarget=()=>isIOS()||isAndroid()||window.matchMedia("(max-width: 820px)").matches;
function showPWAInstall(force=false){
  const card=$("pwaHomeCard");if(!card)return;
  if(isStandalone()||!isMobileInstallTarget()){card.classList.add("hidden");return}
  if(!force&&localStorage.getItem("hr_pwa_dismissed")==="1"){card.classList.add("hidden");return}
  card.classList.remove("hidden");
  const android=$("pwaAndroidBtn");
  if(android){
    if(isIOS()){android.textContent="KURULUM ADIMLARINI GÖR";android.onclick=showIOSInstallHelp}
    else{android.textContent=deferredPWA?"ANDROID'E YÜKLE":"TELEFONA YÜKLE";android.onclick=installPWA}
  }
}
window.addEventListener("beforeinstallprompt",e=>{e.preventDefault();deferredPWA=e;showPWAInstall()});
window.addEventListener("appinstalled",()=>{deferredPWA=null;$("pwaHomeCard")?.classList.add("hidden");localStorage.setItem("hr_pwa_dismissed","1");toast("Hakkari Roll telefona yüklendi 🚗")});
...
async function installPWA(){
  ...
  if(deferredPWA){
    ...
    if(choice?.outcome==="accepted")localStorage.setItem("hr_pwa_dismissed","1");
    deferredPWA=null;showPWAInstall();return;
  }
  toast("Chrome menüsünden ‘Uygulamayı yükle’ veya ‘Ana ekrana ekle’ seçeneğini kullan.");
}
function dismissPWA(){localStorage.setItem("hr_pwa_dismissed","1");$("pwaHomeCard")?.classList.add("hidden")}
function openPWAInstallGuide(){
  localStorage.removeItem("hr_pwa_dismissed");
  showPage("homePage");loadFeed();loadV4Home();showPWAInstall(true);
  setTimeout(()=>$("pwaHomeCard")?.scrollIntoView({behavior:"smooth",block:"center"}),120);
}
