...
let conversationCallHistoryMode=false;
function renderConversations(){const host=$("conversations");if(!host)return;if(conversationCallHistoryMode){setMessageUnreadBadge(conversationCache.reduce((n,c)=>n+(Number(c.unread)||0),0));return;}const q=($("conversationSearch")?.value||"").toLocaleLowerCase("tr");const list=conversationCache.filter(c=>{const o=c.other||{};const hay=`${o.display_name||""} ${o.username||""} ${c.last_message?.body||""}`.toLocaleLowerCase("tr");return (!q||hay.includes(q))&&(!conversationUnreadOnly||c.unread>0)});host.innerHTML=list.map(c=>{const o=c.other||{display_name:"Kullanıcı",username:"",profile_photo:null};const online=o.online||(()=>{const t=parseAppTime(o.last_seen);return !!(t&&Date.now()-t.getTime()<90000)})();return `<button class="conversation v5-conversation ${activeConversation===c.id?"active":""}" data-conversation-id="${c.id}" onclick="openConversation(${c.id},'${esc(o.display_name).replace(/'/g,"&#39;")}')"><span class="v5-conversation-avatar ${online?'online':''}">${img(o.profile_photo)}${online?`<i class="v54-online-dot"></i>`:""}</span><span class="v5-conversation-copy"><b>${esc(o.display_name)}</b><small>@${esc(o.username)}</small><em>${esc(c.last_message?.body||"Yeni sohbet")}</em></span><span class="v5-conversation-meta">${c.unread?`<strong>${c.unread}</strong>`:""}<small>${esc(formatAppTime(c.last_message?.created_at||""))}</small></span></button>`}).join("")||`<div class="v5-empty-conversations"><span>💬</span><p>${conversationUnreadOnly?"Okunmamış mesajın yok.":q?"Aramana uygun sohbet bulunamadı.":"Henüz bir sohbetin yok."}</p><button onclick="openExplore()">YENİ SOHBET BAŞLAT</button></div>`;setMessageUnreadBadge(conversationCache.reduce((n,c)=>n+(Number(c.unread)||0),0))}
function showAllConversations(btn){conversationCallHistoryMode=false;conversationUnreadOnly=false;document.querySelectorAll(".v5-message-tabs button").forEach(x=>x.classList.remove("active"));btn?.classList.add("active");renderConversations()}
function filterUnreadConversations(btn){conversationCallHistoryMode=false;conversationUnreadOnly=true;document.querySelectorAll(".v5-message-tabs button").forEach(x=>x.classList.remove("active"));btn?.classList.add("active");renderConversations()}
async function showCallHistory(btn){conversationCallHistoryMode=true;conversationUnreadOnly=false;document.querySelectorAll('.v5-message-tabs button').forEach(x=>x.classList.remove('active'));btn?.classList.add('active');await loadCallHistory()}
function formatCallDuration(sec){sec=Math.max(0,Number(sec)||0);const m=Math.floor(sec/60),s=sec%60;return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`}
async function loadCallHistory(){const host=$('conversations');if(!host)return;host.innerHTML='<div class="meta">Arama geçmişi yükleniyor...</div>';try{const d=await api('/api/calls/history?limit=80');host.innerHTML=(d.calls||[]).map(c=>{const p=c.peer||{},st=c.status||'',label=voiceStatusText(st),dir=c.is_caller?'Giden':'Gelen',dur=c.duration_seconds?` • ${formatCallDuration(c.duration_seconds)}`:'';return `<article class="v513-call-history"><span class="v513-call-history-avatar">${img(p.profile_photo)}</span><span><b>${esc(p.display_name||p.username||'Kullanıcı')}</b><small>${esc(dir)} • ${esc(label)}${dur}</small><em>${esc(formatAppDateTime(c.created_at))}</em></span><button ${p.can_call?'':`disabled title="${esc(p.call_restriction||'Arama kapalı')}"`} onclick="startVoiceCallUser(${Number(p.id)||0})">📞</button></article>`}).join('')||'<div class="v5-empty-conversations"><span>📞</span><p>Henüz sesli arama geçmişin yok.</p></div>'}catch(e){host.innerHTML=`<div class="v53-message-error"><b>Arama geçmişi yüklenemedi</b><p>${esc(e.message)}</p></div>`}}
...
function callStatusIcon(status){return ({ended:'☎',declined:'↙',missed:'↗',cancelled:'✕'}[status]||'☎')}
function messageBubbleHtml(m){
 const mine=m.sender_id===me.id,deleted=Number(m.deleted_for_everyone)||0;
 if(m.message_type==='call'){
  const status=(m.body||'').toLowerCase().includes('cevapsız')?'missed':(m.body||'').toLowerCase().includes('redded')?'declined':(m.body||'').toLowerCase().includes('iptal')?'cancelled':'ended';
  return `<div class="v513-call-message" data-message-id="${m.id}"><span class="v513-call-message-icon">${callStatusIcon(status)}</span><span><b>${esc(m.body||'Sesli arama')}</b><small>${esc(formatAppTime(m.created_at))}</small></span><button onclick="startVoiceCallFromConversation()">TEKRAR ARA</button></div>`;
 }
 return `<div class="bubble ${mine?"me":""} ${deleted?"v55-deleted-message":""}" data-message-id="${m.id}"><div class="v55-bubble-row"><span class="v55-message-body">${deleted?`<i>🚫 Bu mesaj silindi.</i>`:esc(m.body)}</span><button class="v55-message-menu" onclick="openMessageActions(${m.id},${mine},${deleted});event.stopPropagation()" aria-label="Mesaj işlemleri">⋮</button></div><div class="bubble-time">${esc(formatAppTime(m.created_at))}${mine?` <span class="v54-ticks">${m.read_at?"✓✓":"✓"}</span>`:""}</div></div>`
}
...
const _v512AdminTab=adminTab;adminTab=function(id){_v512AdminTab(id);if(id==='adminVehiclesBox')loadAdminVehicles();if(id==='adminCallsBox')loadAdminCalls();if(id==='adminSystemBox')loadAdminDiagnostics()};
...
let adminCallSearchTimer=null;
function queueAdminCallSearch(){clearTimeout(adminCallSearchTimer);adminCallSearchTimer=setTimeout(loadAdminCalls,220)}
async function loadAdminCalls(){if(me?.role!=='admin')return;const host=$('adminCalls');if(!host)return;host.innerHTML='<div class="meta">Aramalar yükleniyor...</div>';try{const q=encodeURIComponent(($('adminCallSearch')?.value||'').trim()),st=encodeURIComponent($('adminCallStatus')?.value||'all'),d=await api(`/api/admin/calls?q=${q}&status=${st}`);host.innerHTML=(d.calls||[]).map(c=>`<article class="v513-admin-call"><div><b>#${c.id} • ${esc(c.caller_name||c.caller_username||'Kullanıcı')} → ${esc(c.callee_name||c.callee_username||'Kullanıcı')}</b><small>@${esc(c.caller_username)} → @${esc(c.callee_username)}</small><div class="meta">${esc(voiceStatusText(c.status))} • ${esc(formatAppDateTime(c.created_at))}${c.duration_seconds?` • ${formatCallDuration(c.duration_seconds)}`:''}</div></div>${c.active?`<button class="danger" onclick="adminEndCall(${c.id})">ARAMAYI BİTİR</button>`:''}</article>`).join('')||'<div class="meta">Arama kaydı bulunamadı.</div>'}catch(e){host.innerHTML=`<div class="v53-message-error"><b>Aramalar yüklenemedi</b><p>${esc(e.message)}</p></div>`}}
async function adminEndCall(id){if(!(await appConfirm('Aramayı Bitir','Aktif sesli arama iki taraf için kapatılacak.','ARAMAYI BİTİR')))return;try{const d=await api(`/api/admin/calls/${id}/end`,{method:'POST'});toast(d.message);loadAdminCalls();loadAdmin()}catch(e){toast(e.message)}}
async function loadAdminDiagnostics(){const host=$('adminDiagnostics');if(!host)return;host.innerHTML='<div class="meta">Kontrol ediliyor...</div>';try{const d=await api('/api/admin/diagnostics');host.innerHTML=`<div class="panel"><b>✓ Veritabanı</b><div class="meta">${esc(d.database)} • V${esc(d.version||'')}</div></div><div class="panel"><b>Oturum Şeması</b><div class="meta">${(d.session_schema||[]).map(esc).join(' • ')}</div></div><div class="panel"><b>Tablolar</b><div class="meta">${Number(d.table_count)||0} tablo • ${esc(d.timezone||'')}</div></div><div class="panel"><b>📞 TURN / WebRTC</b><div class="meta">${d.turn_configured?'✓ TURN aktif':'⚠ Yalnız STUN'} • ${esc(d.turn_provider||'stun-only')}</div></div>`}catch(e){host.innerHTML=`<div class="v53-message-error"><b>Sistem kontrolü başarısız</b><p>${esc(e.message)}</p></div>`}}
...
// =========================================================
// HAKKARI ROLL V5.13 — SESLİ ARAMA / RECONNECT / TURN
// =========================================================
...
let voiceReconnectTimer=null,voiceReconnectAttempts=0,voiceReconnectBusy=false;
...
function voicePhase(text,tone=''){const e=$('voiceCallPhase');if(!e)return;e.textContent=text||'';e.dataset.tone=tone}
function voiceReconnectInfo(text=''){const e=$('voiceReconnectNote');if(!e)return;e.textContent=text;e.classList.toggle('hidden',!text)}
function voiceOpenOverlay(mode='outgoing'){
 if(!$('notificationsDrawer')?.classList.contains('hidden'))closeNotifications();
 const overlay=$('voiceCallOverlay');if(!overlay||!voiceCall)return;const p=voicePeer();
 $('voiceCallAvatar').innerHTML=voiceAvatarHtml(p);$('voiceCallName').textContent=voicePeerName();$('voiceCallHandle').textContent=p?.username?`@${p.username}`:'';$('voiceCallTimer').textContent='00:00';
 $('voiceIncomingActions').classList.toggle('hidden',mode!=='incoming');$('voiceActiveActions').classList.toggle('hidden',mode==='incoming');voiceReconnectInfo('');
 voicePhase(mode==='incoming'?'GELEN ARAMA':'ARANIYOR',mode==='incoming'?'incoming':'outgoing');$('voiceCallStatus').textContent=mode==='incoming'?'Gelen sesli arama':voiceStatusText(voiceCall.status);
...
function voiceSetAcceptedUi(){
 $('voiceIncomingActions')?.classList.add('hidden');$('voiceActiveActions')?.classList.remove('hidden');voicePhase(voicePc?.connectionState==='connected'?'BAĞLANDI':'BAĞLANIYOR',voicePc?.connectionState==='connected'?'connected':'connecting');
...
function voiceStartTimer(){clearInterval(voiceTimerInterval);const base=parseAppTime(voiceCall?.accepted_at)?.getTime()||Date.now();const tick=()=>{const sec=Math.max(0,Math.floor((Date.now()-base)/1000)),m=Math.floor(sec/60),s=sec%60;if($('voiceCallTimer'))$('voiceCallTimer').textContent=`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`};tick();voiceTimerInterval=setInterval(tick,1000)}
...
async function voiceGetConfig(){if(voiceConfigCache)return voiceConfigCache;try{voiceConfigCache=await api('/api/calls/config')}catch(e){voiceConfigCache={ice_servers:[{urls:'stun:stun.l.google.com:19302'}],turn_configured:false,turn_provider:'stun-only'}}return voiceConfigCache}
async function voicePrepareMic(){if(!voiceSupported())throw new Error('Sesli arama için HTTPS ve mikrofon destekli güncel bir tarayıcı gerekli.');if(voiceLocalStream?.active)return voiceLocalStream;try{voiceLocalStream=await navigator.mediaDevices.getUserMedia({video:false,audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});return voiceLocalStream}catch(e){throw new Error(e?.name==='NotAllowedError'?'Mikrofon izni verilmedi. Sesli arama için mikrofon izni gerekli.':'Mikrofona erişilemedi.')}}
function voiceScheduleReconnect(reason='Bağlantı zayıfladı'){
 if(!voiceCall||voiceCall.status!=='accepted'||voiceCleaning||voiceReconnectTimer)return;if(voiceReconnectAttempts>=2){voicePhase('BAĞLANTI KOPTU','error');voiceSetStatus('Bağlantı yeniden kurulamadı');setTimeout(()=>endVoiceCall(),1400);return}
 voiceReconnectTimer=setTimeout(()=>{voiceReconnectTimer=null;voiceAttemptReconnect(reason)},2500);
}
async function voiceAttemptReconnect(reason='Bağlantı yenileniyor'){
 if(!voiceCall||voiceCall.status!=='accepted'||voiceReconnectBusy)return;voiceReconnectBusy=true;voiceReconnectAttempts++;voicePhase('YENİDEN BAĞLANIYOR','warning');voiceSetStatus(reason);voiceReconnectInfo(`Bağlantı yeniden deneniyor • ${voiceReconnectAttempts}/2`);
 try{const pc=await voiceCreatePeer();if(voiceCall.is_caller){try{pc.restartIce?.()}catch(e){}if(pc.signalingState==='stable'){const offer=await pc.createOffer({iceRestart:true,offerToReceiveAudio:true});await pc.setLocalDescription(offer);await voiceSendSignal('offer',pc.localDescription.toJSON?pc.localDescription.toJSON():pc.localDescription)}}else await voiceSendSignal('restart',{attempt:voiceReconnectAttempts})}catch(e){voiceScheduleReconnect('Bağlantı yeniden deneniyor…')}finally{voiceReconnectBusy=false}
}
async function voiceCreatePeer(){
 if(voicePc)return voicePc;const cfg=await voiceGetConfig();voicePc=new RTCPeerConnection({iceServers:cfg.ice_servers||[],iceCandidatePoolSize:2});(voiceLocalStream?.getTracks()||[]).forEach(t=>voicePc.addTrack(t,voiceLocalStream));
 ...
 const state=()=>{const s=voicePc?.connectionState||voicePc?.iceConnectionState;if(s==='connected'){voiceReconnectAttempts=0;clearTimeout(voiceReconnectTimer);voiceReconnectTimer=null;voiceReconnectInfo('');voicePhase('BAĞLANDI','connected');voiceSetStatus('Bağlandı');voiceSetAcceptedUi()}else if(s==='connecting'||s==='checking'){voicePhase('BAĞLANIYOR','connecting');voiceSetStatus('Bağlanıyor…')}else if(s==='disconnected'){voicePhase('BAĞLANTI ZAYIF','warning');voiceSetStatus('Bağlantı zayıfladı…');voiceScheduleReconnect()}else if(s==='failed'){voicePhase('BAĞLANTI KOPTU','error');voiceSetStatus('Bağlantı kesildi');voiceScheduleReconnect('Bağlantı yeniden kuruluyor…')}};
 voicePc.onconnectionstatechange=state;voicePc.oniceconnectionstatechange=state;return voicePc;
}
...
async function voiceHandleSignal(s){if(!voiceCall||!s)return;const pc=await voiceCreatePeer();try{
 if(s.type==='offer'){if(voiceCall?.status!=='accepted'||voiceCall?.is_caller)return;if(pc.signalingState==='have-local-offer')try{await pc.setLocalDescription({type:'rollback'})}catch(e){}await pc.setRemoteDescription(s.payload);await voiceFlushIce();const ans=await pc.createAnswer();await pc.setLocalDescription(ans);await voiceSendSignal('answer',pc.localDescription.toJSON?pc.localDescription.toJSON():pc.localDescription)}
 else if(s.type==='answer'){if(pc.signalingState==='have-local-offer'){await pc.setRemoteDescription(s.payload);await voiceFlushIce()}}
 ...
 else if(s.type==='restart'&&voiceCall.is_caller&&voiceCall.status==='accepted')voiceAttemptReconnect('Karşı taraf bağlantıyı yeniliyor…');
...
async function voicePollState(){if(!voiceCall)return;try{const previousStatus=voiceCall.status,d=await api(`/api/calls/${voiceCall.id}`),c=d.call;if(!c)return;voiceCall={...voiceCall,...c,peer:c.peer||voiceCall.peer};if(c.status==='accepted'){voiceSetAcceptedUi();if(previousStatus!=='accepted'||!voiceSignalTimer)voiceStartLoops();else await voiceFetchSignals()}else if(c.status==='ringing'&&!voiceCall.is_caller){voicePhase('GELEN ARAMA','incoming');voiceSetStatus('Gelen sesli arama')}else if(['ended','declined','missed','cancelled'].includes(c.status)){voicePhase(c.status==='ended'?'ARAMA BİTTİ':'ARAMA KAPANDI',c.status==='ended'?'':'warning');voiceSetStatus(voiceStatusText(c.status));const msg=c.status==='declined'?'Arama reddedildi.':c.status==='missed'?'Arama cevaplanmadı.':c.status==='cancelled'?'Arama iptal edildi.':'';setTimeout(()=>voiceCleanup(msg),650)}}catch(e){}}
async function startVoiceCallUser(userId){const uid=Number(userId)||0;if(!uid||uid===Number(me?.id))return toast('Kendini arayamazsın.');if(voiceCall)return toast('Zaten aktif bir araman var.');try{const permission=await api(`/api/calls/can/${uid}`);if(!permission.can_call)return toast(permission.reason||'Bu kullanıcıyı arayamazsın.');if(!permission.online)return toast('Bu kullanıcı şu anda çevrimdışı. Çevrimiçi olduğunda arayabilirsin.');await voicePrepareMic();const d=await api('/api/calls',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:uid})});voiceCall=d.call;voiceSignalAfter=0;voicePendingIce=[];voiceMuted=false;voiceRemoteMuted=false;voiceReconnectAttempts=0;voiceOpenOverlay('outgoing');const pc=await voiceCreatePeer(),offer=await pc.createOffer({offerToReceiveAudio:true});await pc.setLocalDescription(offer);await voiceSendSignal('offer',pc.localDescription.toJSON?pc.localDescription.toJSON():pc.localDescription);voiceStartLoops();const cfg=await voiceGetConfig();if($('voiceCallHint'))$('voiceCallHint').textContent=cfg.turn_configured?(cfg.turn_provider==='self-hosted'?'Kendi Hakkari Roll TURN sunucun aktif.':'TURN yedek bağlantısı aktif.'):'Doğrudan WebRTC bağlantısı deneniyor; sıkı mobil ağlarda TURN gerekir.'}catch(e){await voiceCleanup();toast(e.message||'Sesli arama başlatılamadı.')}}
...
async function acceptVoiceCall(){if(!voiceCall||voiceCall.status!=='ringing'||voiceCall.is_caller)return;try{await voicePrepareMic();const d=await api(`/api/calls/${voiceCall.id}/accept`,{method:'POST'});voiceCall=d.call;voiceSignalAfter=0;voicePendingIce=[];voiceReconnectAttempts=0;await voiceCreatePeer();voiceSetAcceptedUi();voiceStartLoops()}catch(e){toast(e.message||'Arama cevaplanamadı.');if(e.message?.includes('Mikrofon'))return;await voiceCleanup()}}
async function declineVoiceCall(){if(!voiceCall)return;try{await api(`/api/calls/${voiceCall.id}/decline`,{method:'POST'})}catch(e){}await voiceCleanup();if(conversationCallHistoryMode)loadCallHistory()}
async function endVoiceCall(silent=false){if(!voiceCall){await voiceCleanup();return}try{await api(`/api/calls/${voiceCall.id}/end`,{method:'POST'})}catch(e){}await voiceCleanup(silent?'':null);if(activeConversation)pollConversationLive();if(conversationCallHistoryMode)loadCallHistory()}
async function voiceCleanup(message=null){if(voiceCleaning)return;voiceCleaning=true;clearInterval(voiceSignalTimer);clearInterval(voiceStateTimer);clearInterval(voiceTimerInterval);clearTimeout(voiceReconnectTimer);voiceSignalTimer=voiceStateTimer=voiceTimerInterval=voiceReconnectTimer=null;try{voicePc?.close()}catch(e){}voicePc=null;try{voiceLocalStream?.getTracks().forEach(t=>t.stop())}catch(e){}voiceLocalStream=null;voicePendingIce=[];voiceSignalAfter=0;voiceSignalBusy=false;voiceReconnectBusy=false;voiceReconnectAttempts=0;voiceMuted=false;voiceRemoteMuted=false;const a=$('voiceRemoteAudio');if(a){try{a.pause()}catch(e){}a.srcObject=null;a.muted=false}voiceCall=null;$('voiceCallOverlay')?.classList.add('hidden');document.documentElement.classList.remove('voice-call-open');document.body.classList.remove('voice-call-open');$('voiceMuteBtn')?.classList.remove('active');$('voiceAudioBtn')?.classList.remove('active');voiceReconnectInfo('');await voiceReleaseWakeLock();voiceCleaning=false;if(message)toast(message)}
...
async function voicePollIncoming(){if(!token||!me||voiceCall||voiceCleaning)return;try{const d=await api('/api/calls/poll'),c=d.call;if(!c)return;if(c.is_caller||c.status==='accepted'){try{await api(`/api/calls/${c.id}/end`,{method:'POST'})}catch(e){}if(c.status==='accepted')toast('Sayfa yenilendiği için önceki sesli arama kapatıldı.');return}voiceCall=c;voiceSignalAfter=0;voicePendingIce=[];if(c.status==='ringing'){voiceOpenOverlay('incoming');voiceStartIncomingRingingLoop();voiceNotifyIncoming(c)}}catch(e){}}
...
async function voiceNotifyIncoming(c){if(!c||voiceLastNotifiedCallId===c.id)return;voiceLastNotifiedCallId=c.id;const name=c.peer?.display_name||c.peer?.username||'Bir kullanıcı';if(('Notification'in window)&&Notification.permission==='granted')await showAppSystemNotification('Gelen sesli arama',`${name} seni arıyor`,{tag:`hr-call-${c.id}`,requireInteraction:true,data:{kind:'call',call_id:c.id,url:'/'}})}
window.addEventListener('offline',()=>{if(voiceCall?.status==='accepted'){voicePhase('İNTERNET YOK','error');voiceSetStatus('İnternet bağlantısı kesildi');voiceReconnectInfo('Bağlantı geri geldiğinde otomatik yeniden denenecek.')}});
window.addEventListener('online',()=>{if(voiceCall?.status==='accepted')voiceAttemptReconnect('İnternet geri geldi, bağlantı yenileniyor…')});
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&voiceCall){voicePollState();voiceFetchSignals();if(voiceCall.status==='accepted'&&voicePc?.connectionState!=='connected')voiceScheduleReconnect('Arama bağlantısı kontrol ediliyor…')}});
