  ...
  try{
    const d=await api("/api/register",{
      ...
      body:JSON.stringify({
        ...
        terms_accepted:$("regTerms").checked,
        safety_accepted:$("regSafety").checked,
        privacy_accepted:$("regPrivacy").checked
      })
    });
    toast(d.message);
    finishAuth(d);
...
// HAKKARI ROLL PROFESSIONAL UX PATCH
...
const _professionalAdminAnnouncement=adminAnnouncement;adminAnnouncement=async function(){await _professionalAdminAnnouncement();loadAdminAnnouncements();loadV4Home()}
const _professionalAdminTab=adminTab;adminTab=function(id){_professionalAdminTab(id);if(id==='adminAnnouncementsBox')loadAdminAnnouncements()}

const _professionalLoadCrews=loadCrews;loadCrews=async function(){await _professionalLoadCrews();try{const d=await api('/api/crews');document.querySelectorAll('#crews .crew-card').forEach((el,i)=>{const c=d.crews[i];if(c?.is_owner&&!el.querySelector('.crew-owner-tools'))el.insertAdjacentHTML('beforeend',`<div class="crew-owner-tools"><button onclick="editCrew(${c.id})">✏️ EKİBİ DÜZENLE</button><button class="danger" onclick="deleteCrew(${c.id})">🗑 EKİBİ SİL</button></div>`)})}catch(e){}}
