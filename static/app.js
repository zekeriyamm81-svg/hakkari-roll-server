...
function clearAppTimers(){appTimers.forEach(clearInterval);appTimers=[];clearInterval(messageTimer);messageTimer=null;if(typeof v4PresenceTimer!=="undefined")clearInterval(v4PresenceTimer);if(typeof crewChatTimer!=="undefined")clearInterval(crewChatTimer)}
function clearSession(){token="";me=null;[localStorage,sessionStorage].forEach(s=>{s.removeItem("hr_token");s.removeItem("hr_user")});clearAppTimers();stopRollWatch()}
function persistUser(){if(!me)return;const remember=localStorage.getItem("hr_remember")==="1";const store=remember?localStorage:sessionStorage;store.setItem("hr_user",JSON.stringify(me))}
function showAuth(message=""){
  clearSession();$("appView")?.classList.add("hidden");$("authView")?.classList.remove("hidden");showLogin();
...
async function api(path,opts={}){
  ...
  if(!res.ok){
    if(res.status===401 && (token||me)){showAuth("Oturumunun süresi doldu. Lütfen yeniden giriş yap.")}
...
async function register(){
  ...
  try{
    ...
    toast(d.message);
    if(d.pending_approval){showLogin();$("loginUser").value=$("regUser").value;return}
    finishAuth(d);
  }catch(e){
    toast(e.message);
  }
}
async function login(){try{const d=await api("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:$("loginUser").value,password:$("loginPass").value,remember:!!$("rememberMe")?.checked})});finishAuth(d)}catch(e){toast(e.message)}}
function finishAuth(d){if(!d?.user){toast("Oturum başlatılamadı.");return}token="";me=d.user;[localStorage,sessionStorage].forEach(s=>{s.removeItem("hr_token");s.removeItem("hr_user")});const remember=!!$("rememberMe")?.checked;localStorage.setItem("hr_remember",remember?"1":"0");const st=remember?localStorage:sessionStorage;st.setItem("hr_user",JSON.stringify(me));showApp()}
async function logout(){try{await api("/api/logout",{method:"POST"})}catch(e){}showAuth()}
...
function buildNav(){
 ...
 const mobileItems=items.filter(x=>["homePage","mapPage","carsPage","messagesPage","profilePage"].includes(x[2]));
 const mobile=$("mobileNav");mobile.innerHTML="";
 mobileItems.forEach(([icon,name,page,fn])=>{
  const b=document.createElement("button");b.type="button";b.dataset.page=page;b.setAttribute("aria-label",name);b.innerHTML=`<i>${icon}</i><span>${name}</span>`;
  b.onclick=()=>{showPage(page);fn?.()};mobile.appendChild(b)
 });
}
function showPage(id){
 document.querySelectorAll(".page").forEach(x=>x.classList.add("hidden"));
 $(id).classList.remove("hidden");
 document.querySelectorAll("#nav button[data-page],#mobileNav button[data-page]").forEach(b=>{const active=b.dataset.page===id;b.classList.toggle("active",active);if(active)b.setAttribute("aria-current","page");else b.removeAttribute("aria-current")});
 document.body.classList.remove("nav-open");
 window.scrollTo({top:0,behavior:"smooth"});
}
async function showApp(){clearAppTimers();$("authView").classList.add("hidden");$("appView").classList.remove("hidden");buildNav();showPage("homePage");await loadVehicleCatalog();loadFeed();loadRollCount();loadNotifications();appTimers.push(setInterval(loadRollCount,15000),setInterval(loadNotifications,20000))}
...
async function bootstrapAuth(){
 try{const d=await api("/api/me");me=d.user;token="";[localStorage,sessionStorage].forEach(s=>s.removeItem("hr_token"));persistUser();await showApp()}
 catch(e){showAuth()}
}
bootstrapAuth();
...
window.addEventListener("load",()=>setTimeout(()=>{if(typeof me!=="undefined"&&me){loadV4Home();v4Presence();v4PresenceTimer=setInterval(v4Presence,60000);appTimers.push(v4PresenceTimer)}},1200));
...
const HR_LOCAL_BRAND_FILES={"Alfa Romeo":"alfa-romeo","Audi":"audi","BMW":"bmw","Chevrolet":"chevrolet","Citroën":"citroen","CUPRA":"cupra","Dacia":"dacia","Fiat":"fiat","Ford":"ford","Honda":"honda","Hyundai":"hyundai","Jeep":"jeep","Kia":"kia","Land Rover":"land-rover","Lexus":"lexus","Mazda":"mazda","Mercedes-Benz":"mercedes-benz","MINI":"mini","Mitsubishi":"mitsubishi","Nissan":"nissan","Opel":"opel","Peugeot":"peugeot","Porsche":"porsche","Range Rover":"range-rover","Renault":"renault","SEAT":"seat","Škoda":"skoda","Suzuki":"suzuki","Tesla":"tesla","Togg":"togg","Toyota":"toyota","Volkswagen":"volkswagen","Volvo":"volvo"};
function brandLogoUrl(brand){
  const key=Object.keys(HR_LOCAL_BRAND_FILES).find(k=>k.toLocaleLowerCase("tr-TR")===String(brand||"").toLocaleLowerCase("tr-TR"));
  return key?`/static/brands/${HR_LOCAL_BRAND_FILES[key]}.svg`:"/static/brands/generic.svg";
}
