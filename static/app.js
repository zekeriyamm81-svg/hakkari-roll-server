...
async function loadRollMap(){try{const d=await api("/api/roll/active");if(!rollMap){rollMap=L.map("rollMap").setView([37.574,43.740],13);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:"© OpenStreetMap"}).addTo(rollMap);rollMarkers=L.layerGroup().addTo(rollMap)}rollMarkers.clearLayers();d.active.forEach(x=>{const car=x.vehicle?`${x.vehicle.brand} ${x.vehicle.model}`:"Araç eklenmemiş",photo=x.profile_photo?`/uploads/${encodeURIComponent(x.profile_photo)}`:"";const mh=photo?`<div class="radar-profile-marker"><img src="${photo}"><i></i></div>`:`<div class="radar-profile-marker radar-no-photo"><span>🚘</span><i></i></div>`;L.marker([x.lat,x.lon],{icon:L.divIcon({className:"radar-div-icon",html:mh,iconSize:[52,58],iconAnchor:[26,54]})}).addTo(rollMarkers).bindPopup(`<div class="radar-popup"><b>${esc(x.display_name)}</b><small>@${esc(x.username)}</small><span>${esc(car)}</span><em>● ${esc(x.status)}</em><div>${x.user_id!==me.id?`<button onclick="openProfile(${x.user_id})">PROFİL</button><button onclick="sendOffer(${x.user_id},'roll')">🔥 ROLL</button>`:`<button onclick="openProfile(${x.user_id})">PROFİLİM</button>`}</div></div>`)});$("activeRollList").innerHTML=d.active.map(x=>`<div class="person roll-user-card"><div class="person-head" onclick="openProfile(${x.user_id})">${img(x.profile_photo)}<div><div class="name">${esc(x.display_name)}</div><div class="meta">${esc(x.vehicle?x.vehicle.brand+" "+x.vehicle.model:"Araç yok")} • <span class="status-live">${esc(x.status)}</span></div></div></div>${x.user_id!==me.id?`<div class="person-actions"><button onclick="openProfile(${x.user_id})">👤 PROFİL</button><button class="cta mini" onclick="sendOffer(${x.user_id},'roll')">🔥 ROLL</button><button onclick="sendOffer(${x.user_id},'piyasa')">🚘 PİYASA</button></div>`:""}</div>`).join("");setTimeout(()=>rollMap.invalidateSize(),100)}catch(e){toast(e.message)}}
...
async function editEvent(id){try{const d=await api('/api/events'),e=d.events.find(x=>x.id===id);if(!e)return toast('Etkinlik bulunamadı.');const z=await appDialog({title:'Etkinliği Düzenle',message:'Buluşma bilgilerini güncelle.',confirmText:'KAYDET',fields:[{name:'title',label:'Etkinlik adı',type:'text',value:e.title||'',required:true,maxLength:120},{name:'event_time',label:'Tarih ve saat',type:'datetime-local',value:(e.event_time||'').replace(' ','T').slice(0,16),required:true},{name:'meeting_text',label:'Buluşma noktası',type:'text',value:e.meeting_text||'',maxLength:200},{name:'description',label:'Açıklama',type:'textarea',value:e.description||'',maxLength:800}]});if(!z)return;const x=await api(`/api/events/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(z)});toast(x.message);loadEvents()}catch(e){toast(e.message)}}


/* V2.4 VIP EXPERIENCE */
function applyVipExperience(){
 if(!me)return;
 const isVip=me.role==="vip"||me.vip===1||me.is_vip===1||me.is_vip===true;
 document.body.classList.toggle("hr-user-vip",isVip);
 const lounge=$("vipLounge");if(lounge)lounge.classList.toggle("hidden",!isVip);
}
const _v24Boot=setInterval(()=>{if(typeof me!=="undefined"&&me){applyVipExperience();clearInterval(_v24Boot)}},250);
function vipBadgeHTML(u){return (u&&(u.role==="vip"||u.vip===1||u.is_vip===1||u.is_vip===true))?'<span class="v24-vip-badge">◆ VIP</span>':""}
