...
let hrPulseTimer=null,hrMapLiveTick=0;
...
async function openNotifications(){const drawer=$("notificationsDrawer");drawer.classList.remove("hidden");document.body.classList.add("notif-open");requestAnimationFrame(()=>drawer.querySelector(".v53-notif-close")?.focus());await loadNotifications();try{await api("/api/notifications/read-all",{method:"POST"});$("notifCount").textContent=""}catch(e){}}function closeNotifications(){const drawer=$("notificationsDrawer");drawer.classList.add("hidden");document.body.classList.remove("notif-open");$("notifBtn")?.focus()}
...
function parseAppTime(v){if(!v)return null;const t=new Date(String(v).replace(" ","T"));return Number.isNaN(t.getTime())?null:t}
function conversationPresence(c){const v=c?.other?.last_seen;if(c?.other?.online)return"🟢 Çevrimiçi";if(!v)return"Özel sohbet";const t=parseAppTime(v);if(t&&Date.now()-t.getTime()<90000)return"🟢 Çevrimiçi";return `Son görülme: ${v}`}
...
function messageBubbleHtml(m){return `<div class="bubble ${m.sender_id===me.id?"me":""}" data-message-id="${m.id}">${esc(m.body)}<div class="bubble-time">${esc(m.created_at)}${m.sender_id===me.id?` <span class="v54-ticks">${m.read_at?"✓✓":"✓"}</span>`:""}</div></div>`}
...
function closeMobileChat(){stopMessageLive();$("messagesPage")?.classList.remove("chat-open");activeConversation=null;lastMessageId=0;loadConversations(true)}
...
async function pollConversationLive(){if(!activeConversation||messageLiveBusy||document.hidden)return;messageLiveBusy=true;const cid=activeConversation;try{const d=await api(`/api/conversations/${cid}/live?after=${lastMessageId}`);if(cid!==activeConversation)return;const host=$("chatMessages"),nearBottom=host.scrollHeight-host.scrollTop-host.clientHeight<130,newMessages=d.messages||[];if(newMessages.length){host.querySelector('.v53-chat-empty')?.remove();host.insertAdjacentHTML('beforeend',newMessages.map(messageBubbleHtml).join(''));lastMessageId=Math.max(lastMessageId,...newMessages.map(m=>Number(m.id)||0));if(nearBottom||newMessages.some(m=>m.sender_id===me.id))host.scrollTop=host.scrollHeight}updateReadTicks(d.read_up_to);const p=$("chatPresence");if(p){if(d.typing){p.textContent='yazıyor…';p.classList.add('typing')}else{p.classList.remove('typing');p.textContent=d.other_online?'🟢 Çevrimiçi':(d.other_last_seen?`Son görülme: ${d.other_last_seen}`:'Özel sohbet')}}if(newMessages.some(m=>m.sender_id!==me.id))loadConversations(true)}catch(e){}finally{messageLiveBusy=false}}
