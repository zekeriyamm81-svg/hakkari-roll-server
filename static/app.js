...
function showPage(id){
 ...
 if(id!=="messagesPage"){clearInterval(messageTimer);messageTimer=null}
...
async function showApp(){
 ...
 if(!window.hrRollCountTimer)window.hrRollCountTimer=setInterval(loadRollCount,15000);
 if(!window.hrNotificationTimer)window.hrNotificationTimer=setInterval(loadNotifications,20000);
}
...
function postHtml(p){
 ...
 return `<article class="post v52-post ${p.author.vip?"vip-card":""}" id="post-${p.id}">
  <div class="post-head v52-post-head">
   <button class="v52-author" onclick="openProfile(${p.author.id})">${img(p.author.profile_photo)}<span><strong>${esc(p.author.display_name)} ${vip(p.author)}</strong><small>@${esc(p.author.username)} • ${esc(p.created_at)}</small></span></button>
...
async function openVehicleDetail(id){
 const modal=$("vehicleDetailModal"),host=$("vehicleDetailContent");modal.classList.remove("hidden");host.innerHTML=`<div class="v52-detail-loading"><span>🚘</span><p>Araç profili yükleniyor...</p></div>`;
 try{const d=await api(`/api/vehicles/${id}`),v=d.vehicle;window.hrOpenVehicle=v;
  const gallery=(v.gallery||[]).map(g=>`<button onclick="openVehicleGalleryImage('${encodeURIComponent(g.photo)}')"><img src="/uploads/${encodeURIComponent(g.photo)}" alt="Araç galerisi"></button>`).join("");
  host.innerHTML=`<div class="v52-detail-hero">${v.photo?`<img src="/uploads/${encodeURIComponent(v.photo)}" alt="${esc(v.brand)} ${esc(v.model)}">`:`<div class="v52-detail-placeholder">🚘</div>`}<button class="v52-detail-close" onclick="closeVehicleDetail()">✕</button><div class="v52-detail-gradient"></div><div class="v52-detail-title"><small>${v.is_favorite?"★ ANA ARAÇ":"HAKKARİ ROLL GARAJ"}</small><h2>${esc(v.brand)} ${esc(v.model)}</h2><p>${esc(v.model_year||"")} ${v.color?`• ${esc(v.color)}`:""}</p></div></div>
   <div class="v52-detail-body">
    ...
    <section class="v52-detail-section"><div class="v52-section-line"><label>GALERİ</label>${v.is_me?`<button onclick="addVehicleGalleryPhoto(${v.id})">+ FOTOĞRAF</button>`:""}</div><div class="v52-gallery">${gallery||`<div class="v52-gallery-empty">Henüz galeri fotoğrafı yok.</div>`}</div></section>
    <div class="v52-vehicle-actions"><button id="vehicleVoteBtn" class="${v.voted?"active":""}" onclick="voteVehicleDetail(${v.id})">🔥 <b id="vehicleVoteCount">${v.votes||0}</b> OY</button>${v.is_me?`<button onclick="makeFavoriteVehicle(${v.id})">★ ANA ARAÇ YAP</button><button onclick="closeVehicleDetail();editVehicle(${v.id})">DÜZENLE</button>`:`<button class="cta" onclick="sendOffer(${v.user_id},'roll')">🔥 ROLL TEKLİFİ</button><button onclick="closeVehicleDetail();startChat(${v.user_id})">💬 MESAJ</button>`}<button onclick="shareVehicle(${v.id},'${esc(v.brand).replace(/'/g,"\\'")} ${esc(v.model).replace(/'/g,"\\'")}')">↗ PAYLAŞ</button></div>
   </div>`;
 }catch(e){host.innerHTML=`<div class="v52-detail-loading"><span>!</span><p>${esc(e.message)}</p></div>`}
}
function closeVehicleDetail(){$("vehicleDetailModal")?.classList.add("hidden")}
...
}async function deleteVehicle(id){if(!(await appConfirm("Aracı Sil","Araç profili ve bu araca ait bilgiler silinecek.","ARACI SİL")))return;try{await api(`/api/vehicles/${id}`,{method:"DELETE"});loadVehicles()}catch(e){toast(e.message)}}
...
async function loadNotifications(){try{const d=await api("/api/notifications");$("notifCount").textContent=d.unread?d.unread:"";$("notifications").innerHTML=d.notifications.map(n=>`<button class="notification v53-notification ${n.read_at?"":"unread"}" onclick="openNotificationTarget('${esc(n.ref_type||"")}',${Number(n.ref_id)||0})"><span class="v53-notification-icon">${notificationIcon(n.type)}</span><span><b>${esc(n.title)}</b><div>${esc(n.body||"")}</div><small>${esc(n.created_at)}</small></span><i>›</i></button>`).join("")||`<div class="v53-notif-empty">Bildirim yok.</div>`}catch(e){}}
async function openNotifications(){$("notificationsDrawer").classList.remove("hidden");document.body.classList.add("notif-open");await loadNotifications();try{await api("/api/notifications/read-all",{method:"POST"});$("notifCount").textContent=""}catch(e){}}function closeNotifications(){$("notificationsDrawer").classList.add("hidden");document.body.classList.remove("notif-open")}
...
function conversationPresence(c){const v=c?.other?.last_seen;if(!v)return"Özel sohbet";const t=new Date(String(v).replace(" ","T"));if(!Number.isNaN(t.getTime())&&Date.now()-t.getTime()<125000)return"🟢 Çevrimiçi";return `Son görülme: ${v}`}
function renderConversations(){
 const host=$("conversations");if(!host)return;
 const q=($("conversationSearch")?.value||"").toLocaleLowerCase("tr");
 const list=conversationCache.filter(c=>{const o=c.other||{};const hay=`${o.display_name||""} ${o.username||""} ${c.last_message?.body||""}`.toLocaleLowerCase("tr");return (!q||hay.includes(q))&&(!conversationUnreadOnly||c.unread>0)});
 host.innerHTML=list.map(c=>{const o=c.other||{display_name:"Kullanıcı",username:"",profile_photo:null};return `<button class="conversation v5-conversation ${activeConversation===c.id?"active":""}" data-conversation-id="${c.id}" onclick="openConversation(${c.id},'${esc(o.display_name).replace(/'/g,"&#39;")}')"><span class="v5-conversation-avatar">${img(o.profile_photo)}</span><span class="v5-conversation-copy"><b>${esc(o.display_name)}</b><small>@${esc(o.username)}</small><em>${esc(c.last_message?.body||"Yeni sohbet")}</em></span><span class="v5-conversation-meta">${c.unread?`<strong>${c.unread}</strong>`:""}<small>${esc(c.last_message?.created_at||"")}</small></span></button>`}).join("")||`<div class="v5-empty-conversations"><span>💬</span><p>${conversationUnreadOnly?"Okunmamış mesajın yok.":q?"Aramana uygun sohbet bulunamadı.":"Henüz bir sohbetin yok."}</p><button onclick="openExplore()">YENİ SOHBET BAŞLAT</button></div>`;
 const total=conversationCache.reduce((n,c)=>n+(Number(c.unread)||0),0);const navBtn=document.querySelector('#nav button[data-page="messagesPage"]');if(navBtn){let badge=navBtn.querySelector('.v53-nav-unread');if(total&&!badge){badge=document.createElement('b');badge.className='v53-nav-unread';navBtn.appendChild(badge)}if(badge){badge.textContent=total>99?'99+':String(total);badge.classList.toggle('hidden',!total)}}
}
...
async function loadConversations(){try{const d=await api("/api/conversations");conversationCache=(d.conversations||[]).filter(c=>c&&c.other);renderConversations()}catch(e){const host=$("conversations");if(host)host.innerHTML=`<div class="v53-message-error"><b>Mesajlar açılamadı</b><p>${esc(e.message)}</p><button onclick="loadConversations()">TEKRAR DENE</button></div>`;toast(e.message)}}
async function openConversation(id,title="Sohbet"){activeConversation=id;const c=conversationCache.find(x=>x.id===id);$("chatTitle").textContent=title;if($("chatPresence"))$("chatPresence").textContent=conversationPresence(c);$("messagesPage")?.classList.add("chat-open");renderConversations();await loadMessages();loadConversations();clearInterval(messageTimer);messageTimer=setInterval(()=>{loadMessages(false);loadConversations()},2500);setTimeout(()=>$('chatInput')?.focus(),80)}
function closeMobileChat(){$("messagesPage")?.classList.remove("chat-open");clearInterval(messageTimer);messageTimer=null}
async function loadMessages(showError=true){if(!activeConversation)return;try{const d=await api(`/api/conversations/${activeConversation}/messages`),host=$("chatMessages");host.innerHTML=d.messages.length?d.messages.map(m=>`<div class="bubble ${m.sender_id===me.id?"me":""}">${esc(m.body)}<div class="bubble-time">${esc(m.created_at)}${m.sender_id===me.id?` ${m.read_at?"✓✓":"✓"}`:""}</div></div>`).join(""):`<div class="v53-chat-empty">Henüz mesaj yok. İlk mesajı sen gönder.</div>`;host.scrollTop=host.scrollHeight}catch(e){if(showError){$("chatMessages").innerHTML=`<div class="v53-message-error"><b>Sohbet açılamadı</b><p>${esc(e.message)}</p><button onclick="loadMessages()">TEKRAR DENE</button></div>`;toast(e.message)}}}
async function sendMessage(){if(!activeConversation){toast("Önce bir sohbet seç.");return}const input=$("chatInput"),body=input.value.trim();if(!body)return;const old=body;input.value="";try{await api(`/api/conversations/${activeConversation}/messages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({body})});await loadMessages();loadConversations()}catch(e){input.value=old;toast(e.message)}}
...
async function editVehicle(id){
 try{const d=await api('/api/vehicles');const v=d.vehicles.find(x=>x.id===id);if(!v)return toast('Araç bulunamadı.');
 const r=await appDialog({title:'Aracı Düzenle',message:'Garajındaki araç bilgilerini güncelle.',confirmText:'DEĞİŞİKLİKLERİ KAYDET',fields:[
 {name:'brand',label:'Marka',type:'text',value:v.brand||'',required:true},{name:'model',label:'Model',type:'text',value:v.model||'',required:true},{name:'model_year',label:'Model yılı',type:'text',value:v.model_year||''},{name:'engine',label:'Motor',type:'text',value:v.engine||''},{name:'plate',label:'Plaka',type:'text',value:v.plate||''},{name:'color',label:'Renk',type:'text',value:v.color||''},{name:'mods',label:'Modifikasyonlar',type:'textarea',value:v.mods||''},{name:'note',label:'Not',type:'textarea',value:v.note||''}]});
 if(!r)return;const x=await api(`/api/vehicles/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(r)});toast(x.message);loadVehicles();if(me)openProfile(me.id)
 }catch(e){toast(e.message)}}
...
document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(!$("notificationsDrawer")?.classList.contains("hidden"))closeNotifications();if(!$("vehicleDetailModal")?.classList.contains("hidden"))closeVehicleDetail();}});
