...
const HR_AGREEMENTS={
traffic:{title:"Güvenli Sürüş ve Trafik Kuralları",html:`<h3>Güvenli sürüş temel şarttır</h3><p>Hakkari Roll bir yarış veya hız denemesi platformu değildir. Hız yapma, makas, drift, yol kapatma, kamuya açık yolda yarış ve trafiği tehlikeye düşüren davranışlar teşvik edilmez.</p><h3>Sürücü sorumluluğu</h3><p>Sürücü trafik mevzuatına, hız sınırlarına, işaretlere ve güvenlik yükümlülüklerine uymalıdır. Uygulama araç hareket halindeyken dikkati dağıtacak biçimde kullanılmamalıdır.</p><h3>Buluşmalar</h3><p>Etkinlikler hukuka uygun, güvenli ve çevreyi rahatsız etmeyecek şekilde düzenlenmelidir. İzinsiz yarış ve tehlikeli gösteriler için etkinlik oluşturulamaz.</p><h3>Sorumluluk</h3><p>Kullanıcının sürüş tercihi, trafik ihlali, kaza, maddi veya bedensel zararlarından uygulama sorumlu değildir. Sosyal özellikler güvenli sürüş yükümlülüğünü kaldırmaz.</p><h3>Yaptırım</h3><p>Tehlikeli sürüşü teşvik eden içerik kaldırılabilir ve hesap kısıtlanabilir.</p>`},
community:{title:"Topluluk ve İçerik Kuralları",html:`<h3>Saygı</h3><p>Hakaret, tehdit, taciz, zorbalık, nefret söylemi, hedef gösterme ve ısrarlı istenmeyen iletişim yasaktır.</p><h3>İçerik</h3><p>Fotoğraf, video, hikâye, yorum ve araç bilgisini paylaşan kullanıcı gerekli paylaşım hakkına sahip olmalıdır. Başkasının özel görüntüsü ve kişisel bilgisi izinsiz yayımlanamaz.</p><h3>Piyasa</h3><p>Yanıltıcı araç bilgisi, sahte satış ve dolandırıcılık yasaktır.</p><h3>Etkinlik</h3><p>Etkinlik bilgileri gerçeğe uygun olmalı; özellik yasa dışı faaliyet veya tehlikeli sürüş organizasyonu için kullanılmamalıdır.</p><h3>Moderasyon</h3><p>İhlal içeren içerikler kaldırılabilir, hesap erişimi sınırlandırılabilir.</p>`},
location:{title:"Konum ve Roll Radar Bilgilendirmesi",html:`<h3>İsteğe bağlı paylaşım</h3><p>Roll Radar yalnızca Roll Aktif açıldığında konumu kullanır. Aktif süre boyunca konum topluluk haritasında gösterilebilir.</p><h3>Kapatma</h3><p>Roll durumu kapatılarak canlı paylaşım sonlandırılabilir.</p><h3>Kötüye kullanım</h3><p>Konum bilgisi takip, taciz, tehdit veya bir kişiyi izinsiz bulma amacıyla kullanılamaz.</p><h3>Hassas konumlar</h3><p>Ev, iş yeri ve özel alanlarda kullanıcı kendi güvenliğini değerlendirmelidir.</p><h3>Sürüş</h3><p>Harita araç hareket halindeyken elle kontrol edilmemelidir.</p>`},
privacy:{title:"Gizlilik ve Hesap Güvenliği",html:`<h3>Hesap</h3><p>Güçlü şifre kullanılmalı ve hesap başkasına kullandırılmamalıdır.</p><h3>Profil</h3><p>Profil adı, kullanıcı adı, fotoğraf, araç bilgileri ve herkese açık paylaşımlar topluluk içinde görüntülenebilir. Plaka için gizlilik seçenekleri kullanılmalıdır.</p><h3>Medya</h3><p>Yüklenen fotoğraf ve videolar uygulamada gösterilmek üzere sunucuya aktarılır. Görüntüdeki kişilerin mahremiyeti gözetilmelidir.</p><h3>Hassas bilgi</h3><p>Telefon, açık adres, kimlik ve finansal bilgiler açık gönderilerde paylaşılmamalıdır.</p><h3>Kabul</h3><p>Hesap oluşturarak güvenli sürüş, topluluk, konum ve gizlilik esaslarına uymayı kabul edersin.</p>`}};
let hrAgreementAccepted={traffic:false,community:false,location:false,privacy:false},hrAgreementCurrent=null;
function openAgreement(k){hrAgreementCurrent=k;let x=HR_AGREEMENTS[k];$("agreementTitle").textContent=x.title;$("agreementText").innerHTML=x.html;$("agreementModal").classList.remove("hidden");let a=$("agreementScroll");a.scrollTop=0;$("agreementAccept").disabled=true;setTimeout(agreementScrolled,80)}
function agreementScrolled(){let a=$("agreementScroll"),ok=a.scrollTop+a.clientHeight>=a.scrollHeight-12;$("agreementAccept").disabled=!ok;$("agreementHint").textContent=ok?"Tamamını okudun. Şimdi onaylayabilirsin.":"Metnin sonuna kadar kaydır."}
function acceptAgreement(){if($("agreementAccept").disabled)return;hrAgreementAccepted[hrAgreementCurrent]=true;$("agreeMark-"+hrAgreementCurrent).textContent="✓ KABUL";$("agreeMark-"+hrAgreementCurrent).classList.add("accepted");closeAgreement();updateAgreementStatus()}
function closeAgreement(){$("agreementModal").classList.add("hidden")}
function updateAgreementStatus(){let n=Object.values(hrAgreementAccepted).filter(x=>!x).length;$("registerSubmitBtn").disabled=n>0;$("agreementStatus").textContent=n?n+" metni daha okumalısın.":"✓ Tüm metinler kabul edildi."}
...
async function register(){
  if(Object.values(hrAgreementAccepted).some(x=>!x)){toast("Önce tüm zorunlu metinleri sonuna kadar okuyup onayla.");return}
  try{
...
function buildNav(){
 const items=[
  ["⌂","Ana Sayfa","homePage",loadFeed],
  ["⌖","Harita","mapPage",loadRollMap],
  ["▰","Garaj","carsPage",loadVehicles],
  ["◎","Piyasa","peoplePage",loadPeople],
  ["⚡","Teklifler","offersPage",loadOffers],
  ["◉","Etkinlik","eventsPage",loadEvents],
  ["♟","Ekipler","crewsPage",loadCrews],
  ["✉","Mesajlar","messagesPage",loadConversations],
  ["●","Profilim","profilePage",()=>openProfile(me.id)]
 ];
 if(me.role==="admin")items.push(["◆","Yönetim","adminPage",loadAdmin]);
 $("nav").innerHTML=`<div class="v2-nav-brand"><img src="/static/icons/icon.svg" alt=""><div><b>HAKKARİ ROLL</b><small>COMMUNITY</small></div></div><div class="v2-nav-items"></div><div class="v2-nav-foot"><small>GÜVENLİ SÜRÜŞ</small><span>Trafik kurallarına uy. Hızını değil, arabanı göster.</span></div>`;
 const host=$("nav").querySelector(".v2-nav-items");
 items.forEach(([icon,name,page,fn])=>{
  const b=document.createElement("button");
  b.dataset.page=page;
  b.innerHTML=`<i>${icon}</i><span>${name}</span>`;
  b.onclick=()=>{showPage(page);fn?.();document.body.classList.remove("nav-open")};
  host.appendChild(b)
 });
}
...
async function loadRollCount(){try{const d=await api("/api/roll/active");$("liveCount").innerHTML=`<i></i> ${d.count} ROLL AKTİF`;if($("v2HeroLive"))$("v2HeroLive").textContent=`${d.count} CANLI`;$("rollToggleBtn").textContent=d.active.some(x=>x.user_id===me.id)?"🟢 ROLL AKTİF":"🟢 ROLL AKTİF ET"}catch(e){}}
