...
async function loadV4Home(){try{const d=await api("/api/v4/dashboard");$("radarCount").textContent=`${d.roll_active} kişi`;$("rollScore").textContent=d.score;$("rollLevel").textContent=d.level;$("announcementStrip").innerHTML=(d.announcements||[]).map(a=>`<div class="announcement-card"><b>📢 ${esc(a.title)}</b><div>${esc(a.body)}</div></div>`).join("");const w=d.weekly_car,wu=d.weekly_user;$("weeklyCar").innerHTML=(wu?`<div class="weekly-user">${img(wu.profile_photo,"avatar big")}<div><div class="eyebrow">⭐ HAFTANIN KULLANICISI</div><div class="name">${esc(wu.display_name)}</div><div class="user">@${esc(wu.username)}</div><button onclick="openProfile(${wu.id})">PROFİLİ GÖR</button></div></div>`:"")+(w?`<div class="weekly-title"><span>🏆 HAFTANIN ARABASI</span><b>${w.votes||0} oy</b></div><div class="weekly-body">${w.photo?`<img src="/uploads/${encodeURIComponent(w.photo)}">`:""}<div><div class="weekly-brand">${brandLogo(w.brand,"brand-logo weekly-brand-logo")}<h2>${esc(w.brand)} ${esc(w.model)}</h2></div><div class="meta">${esc(w.display_name)} • @${esc(w.username)}</div><button class="cta mini" onclick="voteVehicle(${w.id})">🔥 OY VER</button></div></div>`:"")}catch(e){}loadStories()}
...
async function adminWeekly(kind){const id=+(kind==="car"?$("weeklyCarId").value:$("weeklyUserId").value);if(!id){toast("ID gir.");return}try{const d=await api("/api/admin/weekly-feature",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kind,target_id:id})});toast(d.message);loadV4Home()}catch(e){toast(e.message)}}
