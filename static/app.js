...
function vehicleCard(v){return `<article class="car v5-car-card">${v.photo?`<div class="v5-car-photo-wrap"><img src="/uploads/${encodeURIComponent(v.photo)}" alt="${esc(v.brand)} ${esc(v.model)}">${v.user_id===me.id?`<span class="v5-my-car-badge">BENİM ARACIM</span>`:""}</div>`:`<div class="v5-car-photo-wrap v5-car-no-photo"><span>🚘</span>${v.user_id===me.id?`<em>BENİM ARACIM</em>`:""}</div>`}<div class="v5-car-body"><div class="car-title">${esc(v.brand)} ${esc(v.model)}</div><div class="meta">@${esc(v.username)} • ${esc(v.display_name)}</div><div class="v5-car-specs"><span>${esc(v.model_year||"Yıl -")}</span><span>${esc(v.engine||"Motor -")}</span><span>${esc(v.horsepower?`${v.horsepower} HP`:v.fuel||"-")}</span></div>${v.mods?`<div class="meta v5-car-mods">⚙ ${esc(v.mods)}</div>`:""}<div class="person-actions v5-car-actions"><button onclick="openProfile(${v.user_id})">PROFİL</button>${v.user_id!==me.id?`<button class="cta mini" onclick="sendOffer(${v.user_id},'roll')">🔥 ROLL</button>`:""}${v.user_id===me.id||me.role==="admin"?`<button onclick="editVehicle(${v.id})">DÜZENLE</button><button class="danger" onclick="deleteVehicle(${v.id})">SİL</button>`:""}</div></div></article>`}
