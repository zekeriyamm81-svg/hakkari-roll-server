...
function renderMobileMenu(){
 ...
 const go=(label,icon,js,cls="")=>`<button class="${cls}" onclick="closeMobileMenu();${js}"><i>${icon}</i><span>${label}</span><b>›</b></button>`;
 host.innerHTML=go("Ana Sayfa","⌂","showPage('homePage');loadFeed();loadV4Home()")+go("Roll Merkezi","⌖","showPage('rollPage');loadRollHub()")+go("Canlı Harita","◉","showPage('mapPage');loadRollMap()")+go("Roll Teklifleri","⚡","showPage('offersPage');loadOffers()")+go("Garaj","🚘","showPage('carsPage');setGarageMode('mine')")+go("Mesajlar","💬","showPage('messagesPage');loadConversations()")+go("Etkinlikler","◆","showPage('eventsPage');loadEvents()")+go("Ekipler","♟","showPage('crewsPage');loadCrews()")+go("Profilim","●",`openProfile(${me.id})`)+(me.role==="admin"?go("Yönetim","◆","showPage('adminPage');loadAdmin()","admin"):"")+go("Çıkış Yap","↪","logout()","danger");
}
function buildNav(){
 const items=[
