...
def event_update(eid):
 u=me();d=request.get_json() or {};c=db();e=c.execute('select * from events where id=?',(eid,)).fetchone()
 if not e or (e['owner_id']!=u['id'] and u['role']!='admin'):c.close();return jsonify(ok=False,message='Bu etkinliği düzenleme yetkin yok.'),403
 title=str(d.get('title','')).strip()[:120];desc=str(d.get('description','')).strip()[:800];et=str(d.get('event_time','')).strip()[:40];meeting=str(d.get('meeting_text','')).strip()[:200]
 if not title or not et:c.close();return jsonify(ok=False,message='Etkinlik adı ve tarih gerekli.'),400
 c.execute('update events set title=?,description=?,event_time=?,meeting_text=? where id=?',(title,desc,et,meeting,eid));c.commit();c.close();return jsonify(ok=True,message='Etkinlik güncellendi.')
