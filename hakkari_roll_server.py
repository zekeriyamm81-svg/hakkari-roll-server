...
@admin
def stats():
 c=db();s=dict(users=c.execute('select count(*) from users').fetchone()[0],vehicles=c.execute('select count(*) from vehicles').fetchone()[0],posts=c.execute('select count(*) from posts where deleted=0').fetchone()[0],messages=c.execute('select count(*) from messages').fetchone()[0],reports=c.execute('select count(*) from reports where status="open"').fetchone()[0],roll_active=c.execute('select count(*) from roll_presence').fetchone()[0],offers=c.execute('select count(*) from roll_offers').fetchone()[0],events=c.execute('select count(*) from events').fetchone()[0],crews=c.execute('select count(*) from crews').fetchone()[0]);c.close();return jsonify(ok=True,stats=s)
