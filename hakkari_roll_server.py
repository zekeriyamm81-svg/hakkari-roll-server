...
def register():
 d=request.get_json() or {};un=str(d.get('username','')).strip();pw=str(d.get('password',''));dn=str(d.get('display_name','')).strip();terms=bool(d.get('terms_accepted'));safety=bool(d.get('safety_accepted'));privacy=bool(d.get('privacy_accepted'))
 if not (terms and safety and privacy):return jsonify(ok=False,message='Kayıt için Kullanım Koşulları, Gizlilik ve Güvenli Sürüş kurallarını kabul etmelisin.'),400
...
@admin
def admincreate():
 a=me();d=request.get_json() or {};un=str(d.get('username','')).strip();dn=str(d.get('display_name','')).strip();terms=bool(d.get('terms_accepted'));safety=bool(d.get('safety_accepted'));privacy=bool(d.get('privacy_accepted'));email=str(d.get('email','')).strip().lower();pw=str(d.get('password','')).strip() or randpass();role='admin' if d.get('role')=='admin' else 'user';vip=1 if d.get('vip') else 0
