...
def v4_dashboard():
 ...
 if feat_car:
  top=c.execute('select v.*,us.username,us.display_name,count(vv.user_id) votes from vehicles v join users us on us.id=v.user_id left join vehicle_votes vv on vv.vehicle_id=v.id where v.id=? group by v.id',(feat_car['target_id'],)).fetchone()
 else:
  top=c.execute('select v.*,us.username,us.display_name,count(vv.user_id) votes from vehicles v join users us on us.id=v.user_id left join vehicle_votes vv on vv.vehicle_id=v.id group by v.id order by votes desc,v.id desc limit 1').fetchone()
