...
def sendmsg(cid):
 ...
 try:
  ...
  c.execute('insert into messages(conversation_id,sender_id,body,created_at) values(?,?,?,?)',(cid,u['id'],b[:2000],now()));notify(c,oid,'message','Yeni mesaj',f'@{u["username"]}: {b[:80]}','conversation',cid);c.commit();return jsonify(ok=True)
 except sqlite3.DatabaseError as e:
