import os,re,hmac,hashlib,secrets,sqlite3,json,urllib.request,urllib.error
...
APP_VERSION='5.0.0'
APP_ENV=os.environ.get('HAKKARI_ROLL_ENV','development').strip().lower()
...
TRUST_PROXY=os.environ.get('HAKKARI_ROLL_TRUST_PROXY','0').strip()=='1'
MAX_UPLOAD_MB=max(5,min(int(os.environ.get('HAKKARI_ROLL_MAX_UPLOAD_MB','60')),100))
SESSION_HOURS=max(1,min(int(os.environ.get('HAKKARI_ROLL_SESSION_HOURS','12')),168))
SESSION_REMEMBER_HOURS=max(24,min(int(os.environ.get('HAKKARI_ROLL_REMEMBER_HOURS','720')),2160))
PBKDF2_ITERATIONS=max(1000,int(os.environ.get('HAKKARI_ROLL_PBKDF2_ITERATIONS','260000')))
LOGIN_WINDOW_MINUTES=max(1,min(int(os.environ.get('HAKKARI_ROLL_LOGIN_WINDOW_MINUTES','15')),60))
LOGIN_MAX_ATTEMPTS=max(3,min(int(os.environ.get('HAKKARI_ROLL_LOGIN_MAX_ATTEMPTS','8')),30))
SESSION_COOKIE='hr_session'

if IS_PRODUCTION and (ADMIN_PASS=='Admin123!' or PEPPER==b'change-me'):
 raise RuntimeError('Üretimde güçlü HAKKARI_ROLL_ADMIN_PASSWORD ve HAKKARI_ROLL_PEPPER değerleri zorunludur.')

app=Flask(__name__);app.config['MAX_CONTENT_LENGTH']=MAX_UPLOAD_MB*1024*1024
if not IS_PRODUCTION and (ADMIN_PASS=='Admin123!' or PEPPER==b'change-me'):
 app.logger.warning('Geliştirme varsayılanları kullanılıyor. Canlı ortamda admin parolası ve pepper mutlaka değiştirilmelidir.')
...
def phash(p,s=None):
 s=s or secrets.token_hex(16);d=hashlib.pbkdf2_hmac('sha256',p.encode()+PEPPER,bytes.fromhex(s),PBKDF2_ITERATIONS).hex();return s,d
def pok(p,s,d): return hmac.compare_digest(phash(p,s)[1],d)
_DUMMY_SALT='66f677470b26c8b4f8ce23b8d41e6d60'
_DUMMY_HASH=phash('not-a-real-password',_DUMMY_SALT)[1]
def token():
...
def set_auth_cookie(response,raw,remember=False):
 response.set_cookie(SESSION_COOKIE,raw,max_age=SESSION_REMEMBER_HOURS*3600 if remember else None,httponly=True,secure=IS_PRODUCTION,samesite='Strict',path='/')
 return response
def clear_auth_cookie(response):
 response.delete_cookie(SESSION_COOKIE,path='/',secure=IS_PRODUCTION,samesite='Strict')
 return response
def new_session(c,uid,remember=False):
 raw=secrets.token_urlsafe(36);hours=SESSION_REMEMBER_HOURS if remember else SESSION_HOURS;exp=(datetime.now()+timedelta(hours=hours)).strftime('%Y-%m-%d %H:%M:%S')
 c.execute('delete from sessions where user_id=? and (expires_at is not null and expires_at<=?)',(uid,now()))
 c.execute('insert into sessions(token,user_id,created_at,expires_at) values(?,?,?,?)',(token_hash(raw),uid,now(),exp))
 # Bir hesabın ele geçirilmiş/eski cihaz oturumlarını sınırsız biriktirmesini önler.
 old=c.execute('select token from sessions where user_id=? order by created_at desc limit -1 offset 10',(uid,)).fetchall()
 if old:c.executemany('delete from sessions where token=?',[(r['token'],) for r in old])
 return raw,exp
def account_ok(r):
 if not r or not r['active'] or ('approved' in r.keys() and not r['approved']):return False
 b=parse_dt(r['ban_until'] or '')
 return not (b and b>datetime.now())
def me():
 t=token();
 if not t:return None
 c=db();r=c.execute('select u.* from sessions s join users u on u.id=s.user_id where s.token in (?,?) and (s.expires_at is null or s.expires_at>?)',(token_hash(t),t,now())).fetchone();c.close();return r if account_ok(r) else None
def auth(fn):
 @wraps(fn)
 def w(*a,**k):
...
def valid_media_signature(ext,head):
 if ext in ('jpg','jpeg'):return head.startswith(b'\xff\xd8\xff')
 if ext=='png':return head.startswith(b'\x89PNG\r\n\x1a\n')
 if ext=='webp':return len(head)>=12 and head[:4]==b'RIFF' and head[8:12]==b'WEBP'
 if ext in ('heic','heif'):
  return len(head)>=12 and head[4:8]==b'ftyp' and head[8:12] in {b'heic',b'heix',b'hevc',b'hevx',b'mif1',b'msf1'}
 if ext=='webm':return head.startswith(b'\x1aE\xdf\xa3')
 if ext in ('mp4','mov','m4v'):return len(head)>=12 and head[4:8]==b'ftyp'
 return False
def save(f,prefix,allowed=None):
 ...
 head=f.stream.read(32);f.stream.seek(0)
 if not valid_media_signature(ext,head):raise ValueError('Dosyanın içeriği seçilen medya biçimiyle eşleşmiyor.')
...
def pub(u,viewer=None):
 is_me=u['id']==viewer
 return dict(id=u['id'],username=u['username'],display_name=u['display_name'],bio=u['bio'] or '',profile_photo=u['profile_photo'],role=u['role'],vip=bool(u['vip']) if 'vip' in u.keys() else False,email_verified=bool(u['email_verified']) if is_me and 'email_verified' in u.keys() else None,email=(u['email'] or '') if is_me and 'email' in u.keys() else '',is_me=is_me)
...
def init():
 ...
 x.execute('create table if not exists sessions(token text primary key,user_id integer,created_at text,expires_at text,foreign key(user_id) references users(id) on delete cascade)')
 ...
 ensure_column(c,'email_verifications','attempts','integer not null default 0')
 ...
 ensure_column(c,'password_resets','attempts','integer not null default 0')
 ...
 x.execute('create table if not exists login_attempts(id integer primary key autoincrement,ip text,username text,success integer default 0,created_at text)')
 ...
 ensure_column(c,'sessions','expires_at','text')
 x.execute('create index if not exists idx_sessions_user on sessions(user_id)')
 x.execute('create index if not exists idx_sessions_expires on sessions(expires_at)')
 x.execute('create index if not exists idx_login_attempts_lookup on login_attempts(ip,username,created_at)')
 x.execute('create index if not exists idx_notifications_user on notifications(user_id,created_at)')
 x.execute('create index if not exists idx_posts_feed on posts(deleted,id)')
 legacy_exp=(datetime.now()+timedelta(hours=SESSION_REMEMBER_HOURS)).strftime('%Y-%m-%d %H:%M:%S')
 x.execute('update sessions set expires_at=? where expires_at is null or expires_at=""',(legacy_exp,))
 x.execute('delete from sessions where expires_at<=?',(now(),))
 cleanup_before=(datetime.now()-timedelta(days=2)).strftime('%Y-%m-%d %H:%M:%S')
 x.execute('delete from login_attempts where created_at<?',(cleanup_before,))
...
def client_ip():
 x=request.headers.get('X-Forwarded-For','') if TRUST_PROXY else ''
 return (x.split(',')[0].strip() if x else request.remote_addr or 'unknown')[:80]
...
def no_cache_dynamic_assets(response):
 try:
  p=request.path
  if p.startswith('/api/') or p in ('/service-worker.js','/static/service-worker.js','/static/app.js','/static/app.css','/'):
  ...
  if IS_PRODUCTION and request.is_secure:response.headers['Strict-Transport-Security']='max-age=31536000; includeSubDomains'
 except Exception:
  pass
 return response

@app.errorhandler(413)
def too_large(_):return jsonify(ok=False,message=f'Dosya çok büyük. En fazla {MAX_UPLOAD_MB} MB yükleyebilirsin.'),413

@app.errorhandler(404)
def api_not_found(_):
 if request.path.startswith('/api/'):return jsonify(ok=False,message='API adresi bulunamadı.'),404
...
def health():
 try:
  c=db();c.execute('select 1').fetchone();c.close();return jsonify(ok=True,service='Hakkari Roll',version=APP_VERSION,database='ok')
 except sqlite3.Error:
  return jsonify(ok=False,service='Hakkari Roll',version=APP_VERSION,database='unavailable'),503
...
def register():
 ...
 if len(pw)<10 or len(dn)<2:return jsonify(ok=False,message='Ad ve en az 10 karakter şifre gerekli.'),400
 ...
 approved=0 if mode=='approval' else 1;salt,h=phash(pw)
 cur=c.execute('insert into users(username,password_salt,password_hash,display_name,email,active,approved,email_verified,created_at) values(?,?,?,?,?,1,?,0,?)',(un,salt,h,dn,'',approved,now()))
 c.execute('insert into registration_attempts(ip,email,created_at) values(?,?,?)',(ip,'',now()))
 uid=cur.lastrowid
 if not approved:
  c.commit();c.close();return jsonify(ok=True,pending_approval=True,message='Başvurun alındı. Yönetici onayından sonra giriş yapabilirsin.')
 remember=bool(d.get('remember',True));t,exp=new_session(c,uid,remember);c.commit();u=c.execute('select * from users where id=?',(uid,)).fetchone();c.close()
 return set_auth_cookie(jsonify(ok=True,token=t,expires_at=exp,user=pub(u,uid),message='Hesap oluşturuldu. İşlem yapmak için e-postanı doğrulaman gerekecek.'),t,remember)

@app.post('/api/login')
def login():
 d=request.get_json() or {};un=str(d.get('username','')).strip();pw=str(d.get('password',''));ip=client_ip();c=db();since=(datetime.now()-timedelta(minutes=LOGIN_WINDOW_MINUTES)).strftime('%Y-%m-%d %H:%M:%S')
 failures=c.execute('select count(*) from login_attempts where ip=? and username=? collate nocase and success=0 and created_at>=?',(ip,un,since)).fetchone()[0]
 if failures>=LOGIN_MAX_ATTEMPTS:
  c.close();response=jsonify(ok=False,message=f'Çok fazla başarısız deneme. {LOGIN_WINDOW_MINUTES} dakika sonra tekrar dene.');response.status_code=429;response.headers['Retry-After']=str(LOGIN_WINDOW_MINUTES*60);return response
 u=c.execute('select * from users where username=? collate nocase',(un,)).fetchone();valid=pok(pw,u['password_salt'],u['password_hash']) if u else pok(pw,_DUMMY_SALT,_DUMMY_HASH)
 if u is None or not valid:
  c.execute('insert into login_attempts(ip,username,success,created_at) values(?,?,0,?)',(ip,un,now()));c.commit();c.close();return jsonify(ok=False,message='Kullanıcı adı veya şifre yanlış.'),401
 if not u['approved']:c.close();return jsonify(ok=False,code='APPROVAL_REQUIRED',message='Hesabın henüz yönetici tarafından onaylanmadı.'),403
 ...
 remember=bool(d.get('remember'));c.execute('delete from login_attempts where ip=? and username=? collate nocase',(ip,un));t,exp=new_session(c,u['id'],remember);c.execute('update users set last_login=? where id=?',(now(),u['id']));c.commit();c.close();return set_auth_cookie(jsonify(ok=True,token=t,expires_at=exp,user=pub(u,u['id'])),t,remember)
@app.post('/api/logout')
@auth
def logout():
 t=token();c=db();c.execute('delete from sessions where token in (?,?)',(token_hash(t),t));c.commit();c.close();return clear_auth_cookie(jsonify(ok=True))
...
def email_verification_request():
    ...
    if recent and parse_dt(recent['created_at']) and parse_dt(recent['created_at']) > one_minute_ago:
        c.close()
        return jsonify(ok=False, message='Yeni kod istemeden önce 1 dakika bekle.'), 429
...
def email_verification_confirm():
    ...
    if not hmac.compare_digest(incoming_hash, row['code_hash']):
        ...
        c.execute('UPDATE email_verifications SET attempts=?, used=? WHERE id=?',(attempts,1 if attempts>=5 else 0,row['id']))
        c.commit()
        c.close()
        return jsonify(ok=False, message='Doğrulama kodu yanlış.' if attempts<5 else 'Çok fazla hatalı deneme. Yeni kod iste.'), 400
...
def reset_request():
 ...
 recent=c.execute('select created_at from password_resets where user_id=? order by id desc limit 1',(u['id'],)).fetchone()
 if recent and parse_dt(recent['created_at']) and parse_dt(recent['created_at'])>datetime.now()-timedelta(minutes=1):c.close();return jsonify(ok=False,message='Yeni kod istemeden önce 1 dakika bekle.'),429
...
def reset_confirm():
 ...
 if len(pw)<10:return jsonify(ok=False,message='Yeni şifre en az 10 karakter.'),400
 ...
 if not hmac.compare_digest(ch,r['code_hash']):
  attempts=(r['attempts'] or 0)+1;c.execute('update password_resets set attempts=?,used=? where id=?',(attempts,1 if attempts>=5 else 0,r['id']));c.commit();c.close();return jsonify(ok=False,message='Kod yanlış.' if attempts<5 else 'Çok fazla hatalı deneme. Yeni kod iste.'),400
...
@auth
def getme():
 u=me();response=jsonify(ok=True,user=pub(u,u['id']))
 # V4'ten kalan Bearer oturumlarını ilk başarılı kontrolde HttpOnly cookie'ye taşır.
 if request.headers.get('Authorization','').lower().startswith('bearer '):set_auth_cookie(response,token(),False)
 return response
@app.put('/api/me')
@auth
def editme():
 u=me();d=request.get_json() or {};dn=str(d.get('display_name',u['display_name'])).strip()[:80];bio=str(d.get('bio',u['bio'] or '')).strip()[:500]
 if len(dn)<2:return jsonify(ok=False,message='Görünen ad en az 2 karakter olmalı.'),400
 c=db();c.execute('update users set display_name=?,bio=? where id=?',(dn,bio,u['id']));c.commit();r=c.execute('select * from users where id=?',(u['id'],)).fetchone();c.close();return jsonify(ok=True,user=pub(r,r['id']))
...
def admincreate():
 ...
 if email and not re.fullmatch(r'[^@\s]+@[^@\s]+\.[^@\s]+',email):return jsonify(ok=False,message='E-posta adresi geçersiz.'),400
 if len(pw)<10:return jsonify(ok=False,message='Şifre en az 10 karakter olmalı.'),400
 c=db();
 if c.execute('select 1 from users where username=? collate nocase',(un,)).fetchone():c.close();return jsonify(ok=False,message='Kullanıcı adı kullanılıyor.'),409
 if email and c.execute('select 1 from users where email=? collate nocase',(email,)).fetchone():c.close();return jsonify(ok=False,message='E-posta adresi kullanılıyor.'),409
...
def adminapprove(uid):
 a=me();c=db();u=c.execute('select * from users where id=?',(uid,)).fetchone()
 if not u:c.close();return jsonify(ok=False,message='Hesap bulunamadı.'),404
 c.execute('update users set approved=1,active=1 where id=?',(uid,));audit(c,a['id'],'Hesap onaylandı','user',uid,u['username']);c.commit();c.close();return jsonify(ok=True,message='Hesap onaylandı.')
@app.post('/api/admin/users/<int:uid>/vip')
@admin
def adminvip(uid):
 a=me();c=db();u=c.execute('select * from users where id=?',(uid,)).fetchone()
 if not u:c.close();return jsonify(ok=False,message='Hesap bulunamadı.'),404
 nv=0 if u['vip'] else 1;c.execute('update users set vip=? where id=?',(nv,uid));audit(c,a['id'],'VIP değiştirildi','user',uid,str(nv));c.commit();c.close();return jsonify(ok=True,vip=bool(nv),message='VIP güncellendi.')
...
def admintoggle(uid):
 a=me();c=db();u=c.execute('select * from users where id=?',(uid,)).fetchone()
 if not u:c.close();return jsonify(ok=False,message='Hesap bulunamadı.'),404
 nv=0 if u['active'] else 1;c.execute('update users set active=? where id=?',(nv,uid));c.execute('delete from sessions where user_id=?',(uid,)) if not nv else None;audit(c,a['id'],'Aktif/Pasif','user',uid,str(nv));c.commit();c.close();return jsonify(ok=True,active=bool(nv))
@app.post('/api/admin/users/<int:uid>/reset-password')
@admin
def adminreset(uid):
 a=me();d=request.get_json() or {};pw=str(d.get('password','')).strip() or randpass()
 if len(pw)<10:return jsonify(ok=False,message='Şifre en az 10 karakter olmalı.'),400
 salt,h=phash(pw);c=db()
 if not c.execute('select 1 from users where id=?',(uid,)).fetchone():c.close();return jsonify(ok=False,message='Hesap bulunamadı.'),404
 c.execute('update users set password_salt=?,password_hash=? where id=?',(salt,h,uid));c.execute('delete from sessions where user_id=?',(uid,));audit(c,a['id'],'Şifre sıfırlandı','user',uid,'');c.commit();c.close();return jsonify(ok=True,password=pw,message='Şifre sıfırlandı.')
